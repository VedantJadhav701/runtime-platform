...
export function RepairHUD({ className, active = true }: RepairHUDProps) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-runtime-border/50 bg-runtime-card/30">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] font-bold tracking-widest text-runtime-muted uppercase">
            repair_hud / autonomous_recovery
          </span>
        </div>
        <TelemetryPulse bars={4} color={active ? "amber" : "blue"} className="scale-75" />
      </div>

      {/* Log Output */}
      <div className="p-4 space-y-1 min-h-[200px] max-h-[280px] overflow-hidden">
        <AnimatePresence>
          {repairSequence.slice(0, visibleLines).map((entry, i) => (
            <motion.div
              ...
              className="flex gap-3 text-[10px] leading-relaxed"
            >
      ...
      {/* Status Bar */}
      <div className="px-4 py-2 border-t border-runtime-border/50 bg-runtime-card/20 flex items-center justify-between">
